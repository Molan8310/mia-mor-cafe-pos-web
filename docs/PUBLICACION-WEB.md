...
https://mia-mor-cafe-pos-web.onrender.com
...
- El punto de venta muestra Efectivo, Tarjeta, Transferencia y Mixto.
- La version corregida incluye impresion automatica de ticket al cobrar y reimpresion desde Ventas.
...
Estado actual:
- Desplegado en Render Free.
- Base de datos conectada a Neon PostgreSQL mediante `DATABASE_URL`.
- API validada en `/api/health`.
- Login administrador/licenciante validado.
- Login administrador POS inicial validado.
- Punto de venta local validado con venta, metodo de pago, ticket y cancelacion en base temporal.
